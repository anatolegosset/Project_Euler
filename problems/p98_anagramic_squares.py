...
from pe_lib import check_gonal
    ...
    for i in range(1, 10):
        for j, word1 in enumerate(words_dict[i]):
            for word2 in words_dict[i][j + 1:]:
                if Counter(word1) == Counter(word2) and word1 not in ['SHEET', 'THESE', 'CENTRE', 'RECENT', 'EXCEPT', 'EXPECT', 'FORMER', 'REFORM']:
    ...
    for i in range(int(math.sqrt(10 ** 9))):
        ...
        if len(set(square_str)) < nb_char:
            ...
            for permutation in anagram_perms[nb_char]:
                permuted = int(''.join([square_str[permutation[j]] for j in range(nb_char)]))
                if check_gonal(permuted, 4):
                    if max(permuted, square) > largest_square and len(str(permuted)) == len(square_str):
